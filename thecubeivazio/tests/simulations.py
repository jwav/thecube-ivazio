...
def valid_simulation():
    ...
    test_eq(lambda: MASTER.teams.find_team_by_name(team_name).current_cubebox_id , cube_id,
         "Team should be associated with the cubebox")
    test_eq(lambda: MASTER.teams.find_team_by_name(team_name).completed_cubeboxes, [],
         "Team should not have completed any cubeboxes")
    test_eq(lambda: str(MASTER.teams.find_team_by_name(team_name).max_time_sec), str(max_time),
