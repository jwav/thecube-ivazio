...
if __name__ == "__main__":
    ...
    exit(0)
    config.log.ack_info("-----------------")
    config.log.ack_info(f"config valid?: {config.is_valid()}")
    config.log.ack_info(f"game_durations: {config.game_durations_str}")
    config.log.ack_info(f"game_durations: {config.game_durations_sec}")
    config.log.ack_info(f"team_names: {config.defined_team_names}")
    config.log.ack_info(f"trophies: {config.defined_trophies}")
